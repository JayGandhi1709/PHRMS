...
const handleError = (err) => {
  ...
  if (err.message === "Suspended") {
    errors = "Wait till your suspension is removed";
  }
...
module.exports.doctor_register = async (req, res) => {
  ...
  if (doctorEmailExists) {
    ...
    try {
      if (req.files) {
        let file = req.files;
        file.doc = Array.isArray(file.doc) ? file.doc : [file.doc];
        // file.doc.forEach(async (element, i) => {
        ...
        //   try {
        //     obj.EducationInformation.educationList[i].doc = "element.name";
        //   } catch (e) {
        //     res.status(404).json({
        ...
        for (let [key] of Object.keys(file.doc)) {
          // write a code for upload file in cloudinary
          console.log(file.doc[key].tempFilePath);
          ...
          try {
            ...
            obj.EducationInformation.educationList[key].doc = result.url;
          } catch (e) {
            res.status(404).json({
              errors: "Something went wrong!Please try again later!!",
            });
